...
export const studiofySite = async ({
  repoName: siteName,
  ...
  try {
    ...
    const siteId = await prepareSite(client, siteName, useStagingBranch);
...
export const prepareSite = async (
  ...
  // Step 3: Download the site from GitHub
  const tarballUrl = await octokit.rest.repos.downloadTarballArchive({
    owner: "isomerpages",
    repo: siteName,
    ref: useStagingBranch ? "staging" : "master",
  });
  const tarballResponse = await fetch(tarballUrl.url);

  // Step 4: Unzip the downloaded tarball into the cloned repository
  // then copy over the images/ and files/ folders into the public/ folder
  ...
  const publicDir = path.join(cloneDir, siteName, "public");
  ...
  await fs.promises.cp(conversionDir, schemaDir, { recursive: true });
