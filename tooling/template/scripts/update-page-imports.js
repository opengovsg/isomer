        ...
        for (const specifier of node.specifiers) {
          if (n.ImportSpecifier.check(specifier)) {
            ...
            if (isLayoutImport) {
              ...
              for (const [componentType, componentImportName] of Object.entries(COMPONENT_IMPORTS)) {
                if (importName === componentImportName && usedComponents.has(componentType)) {
                  shouldKeep = true
                  break
                }
              }
...
const processSwitchInFunction = (funcBody, funcName, usedLayouts, usedComponents) => {
  ...
  recast.visit(ast, {
    ...
    visitFunctionDeclaration(path) {
      ...
      if (funcName === "renderNextLayout" || funcName === "renderComponent") {
        if (path.node.body && n.BlockStatement.check(path.node.body)) {
          processSwitchInFunction(path.node.body, funcName, usedLayouts, usedComponents)
        }
      }
    ...
    visitVariableDeclarator(path) {
      ...
      if (varName === "renderNextLayout" || varName === "renderComponent") {
        ...
        if (n.ArrowFunctionExpression.check(init) && 
            n.BlockStatement.check(init.body)) {
          processSwitchInFunction(init.body, varName, usedLayouts, usedComponents)
        }
        
        // Handle function expressions: function() { ... }
        if (n.FunctionExpression.check(init) && 
            n.BlockStatement.check(init.body)) {
          processSwitchInFunction(init.body, varName, usedLayouts, usedComponents)
        }
      }
...
const processPageFile = async (filePath, sitemapAnalysis) => {
  ...
  const { usedLayouts, usedComponents } = collectUsedItems(sitemapAnalysis, routePath)
