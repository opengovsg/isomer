...
describe("buildFeedXml", () => {
  it("builds channel metadata and one item per collection item", () => {
    ...
    expect(xml).toContain(
      '<guid isPermaLink="false">urn:isomer:resource:link</guid>',
    )
    expect(xml).toContain(
  ...
  it("omits <link> rather than emitting an unsafe href", () => {
    // Arrange — getReferenceLinkHref strips these schemes and returns undefined
    const site = makeSite([
      makeItem({
        id: "xss",
        permalink: `${COLLECTION_PERMALINK}/xss`,
        title: "Dodgy link",
        layout: "link",
    ...
    // Assert — the item still appears, but carries no link at all
    expect(xml).toContain("<title>Dodgy link</title>")
    expect(xml).not.toContain("javascript:")
    expect(xml).not.toMatch(/<item>.*<link>/s)
  })
