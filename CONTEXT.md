...
**Export Batch** (`AuditLogExportBatch`): The delivery state for one "allSites" ask that isn't per-site — the assembled zip's location and whether it's been emailed. Correlates with its sibling Export Requests by `batchId`, but is its own row: an Export Batch's zip bundles every sibling's successfully-generated Export Artifact, named so the source site is unambiguous, and is delivered by one Download Token instead of one per site.
_Avoid_: batch (the adjective describing the "allSites" ask itself; the Export Batch is specifically its delivery-state row)

**Download Token**: The sealed, tamper-proof credential embedded in the export-ready email link. Identifies exactly one Audit Log Export request, or — for an "allSites" ask — one Export Batch; possession of the link suffices to download — no Studio login. Opaque to the recipient and unforgeable.
...
**Download Window**: The period during which a Download Token is honoured — 3 days from fulfilment, not from the CSV/zip's creation. Anchored to the request's `completedAt` for a single-site Download Token, or the Export Batch's `emailedAt` for a batch one (a batch has no single per-row completion instant). Under Complete-Artifact reuse, each request gets its own Download Window even when several requests share one CSV.
