...
5. Pre-flight audit (optional but recommended):
   - `psql "$DATABASE_URL" -f prisma/scripts/findCollectionsWithLegacyTags.sql` — per-collection roll-up of items that still carry usable legacy `page.tags` on sides without `tagged`.
   - `psql "$DATABASE_URL" -f prisma/scripts/findItemsWithLegacyTags.sql` — item-level detail (draft/published sides) for the same unmigrated inventory.
