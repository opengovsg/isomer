...
| `SESSION_SECRET`  | A sequence of random characters used to protect session identifiers, generated by running `bunx uuid` from your terminal | 66a21b98-fb17-4259-ac4f-e94d303ac894                                        |
...
export $(grep DATABASE_URL .env.development.local | xargs) && bun run setup
```
...
Next, run the following command: `bun run jump:vapt`. This sets up the port-forwarding service.
Finally, run the following command in a separate terminal: `bun run migrate` to run the migration.
