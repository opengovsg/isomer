...
describe("getMonthOptions", () => {
  it("returns the current month first, newest to oldest, with no future months", () => {
    ...
    expect(options[0]).toEqual({ label: "June 2026", value: "2026-06" })
    expect(options[1]).toEqual({ label: "May 2026", value: "2026-05" })
    expect(options.at(-1)).toEqual({ label: "July 2025", value: "2025-07" })
  ...
  it("uses Singapore time at the UTC day boundary", () => {
    ...
    expect(options[0]).toEqual({ label: "July 2026", value: "2026-07" })
  })
  ...
  it("caps the option count when maxMonths is smaller than the full window", () => {
    ...
    expect(options[0]).toEqual({ label: "June 2026", value: "2026-06" })
    expect(options.at(-1)).toEqual({ label: "April 2026", value: "2026-04" })
  })
