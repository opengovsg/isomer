...
describe("ScheduledPublishingModal.utils", () => {
  describe("getEarliestAllowableTime", () => {
    ...
    it("should return the correct earliest allowable time for same day", () => {
      ...
      const hoursToAdd = 10
      // 10 hours later
      const earliestSchedule = addHours(FIXED_NOW, hoursToAdd)
      // 10:00 AM UTC
      const selectedDate = FIXED_NOW
      // same day
    ...
    it("should return null for different days", () => {
      ...
      const earliestScheduleDay = addDays(FIXED_NOW, 1)
      // next day
      const hoursToAdd = 10
      // 10 hours later
      const earliestSchedule = addHours(earliestScheduleDay, hoursToAdd)
      // 10:00 AM
