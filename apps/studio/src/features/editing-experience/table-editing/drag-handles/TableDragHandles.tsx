...
import type { Axis } from "~/features/editing-experience/table-editing/axis/types"
...
} from "~/features/editing-experience/table-editing/chrome/tableEditorChrome"
...
export const TableDragHandles = ({
  ...
  const renderAxisHandles = (geometry: TableGeometry, axis: Axis) => {
    ...
    const table = getTableAt(editor.state.doc, geometry.pos)
    const mapped = table ? toMappedTable(table) : null
    ...
    return rects.map((rect, index) => {
      ...
      return (
        <AxisHandle
          ...
          index={index}
          isLocked={mapped ? isSlotLocked(axis, index, mapped) : false}
