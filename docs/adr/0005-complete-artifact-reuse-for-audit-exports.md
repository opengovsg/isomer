...
An Export Request whose `(site, Export Range, report type)` matches an existing **Complete Artifact** — one whose data was frozen *after* its range had fully elapsed (`completedAt >= rangeEnd`) — is fulfilled by re-delivering that artifact with a freshly signed URL, not by regenerating the CSV and not by erroring. On the generate path `completedAt` carries an instant captured immediately *before* the report query runs, never the job-finish time: finishing happens only after upload and email, so a current-month job that crosses SGT midnight — or spends time in retries — between querying and finishing would otherwise advertise a permanently incomplete CSV as complete. (The reuse path stamps delivery time, which is sound because reuse only ever hands out an already-complete artifact.) Duplicate asks while a job is in flight are accepted idempotently (no error, no second request row). The invariant is *the system never does duplicate work*; the completeness predicate is what makes that safe, because audit records are append-only, so a Complete Artifact's content can never change.
...
- **Naive range-equality reuse** — rejected for the *last-day trap*: an export of the current month generated on its final day carries the full-month range but is missing that day's later events; range equality alone would let that snapshot masquerade as the authoritative month export forever. Hence the `completedAt >= rangeEnd` predicate (with `completedAt` captured pre-query, as above) — in-progress-month exports (clamped ranges) are point-in-time snapshots and always regenerate, which is not duplicate work because the data is still growing.
