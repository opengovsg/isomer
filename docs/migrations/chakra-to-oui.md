...
| `PublishButton` (`Button` + `MenuButton as={IconButton}` split button, wrapped in `TouchableTooltip`) | **Deferred to the Menu/Tooltip unit.** The standalone `Button` is one half of a split button joined to a Chakra `MenuButton` (shared `borderRightRadius={0}`/`borderLeftRadius={0}` seam) and the whole control is a `TouchableTooltip` trigger — migrating the half alone breaks the seam + tooltip wiring. Resolve when the Menu unit converts the split button to `<MenuTrigger>`. |
| `JsonFormsSocialMediaControl` "Back to footer" (`Button variant="link"` + `textStyle="subhead-2"`) | **Deferred (link-variant + font override).** Kept on Chakra (`ChakraButton` alias). `variant="link"` maps to the `LinkButton` bridge, but the `textStyle="subhead-2"` font override needs a verified Tailwind mapping and the edit-item sub-view has no story to gate it. Migrate with a story or in the polish pass. |
| Tooltip-coupled `Add a link` triggers (`JsonFormsNavbarControl`, `JsonFormsSocialMediaControl`) | **Migrate with the Tooltip unit.** Left as `ChakraButton` aliases — they are `<Tooltip>` trigger children (Chakra clones them to inject `onMouseEnter`/`ref`); a react-aria Button ignores that. Resolve to plain OUI `Button` under an OUI tooltip when the Tooltip unit runs. |
