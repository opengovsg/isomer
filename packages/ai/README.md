...
- **`createPairFoundryClient({ apiKey })`** — returns a client bound to `https://engine.pair.gov.sg`:
  - `generateText({ modelId, system?, prompt, maxOutputTokens?, abortSignal? })` — one completion. `prompt` is the user message content. `maxOutputTokens` and `abortSignal` are forwarded only when the caller sets them. Throws when the model returns empty text.
