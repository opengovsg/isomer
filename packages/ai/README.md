...
- **`createFoundryClient({ apiKey })`** — returns a client bound to `https://engine.pair.gov.sg`:
  - `generateText({ modelId, system, prompt, imageUrl?, maxOutputTokens? })` — one completion. `maxOutputTokens` is forwarded only when the caller sets it. When `imageUrl` is set, the user message is a text part plus `image: new URL(imageUrl)`. Throws when the model returns empty text.
