...
describe("ChildrenPages.utils", () => {
  describe("createChildrenPagesComparator", () => {
    it("should sort the children pages in the order specified", () => {
      ...
      const actual = structuredClone(generateChildrenPages()).toSorted(
        comparator,
      )
    ...
    it("should precompute the map for O(1) lookups", () => {
      ...
      const sorted1 = structuredClone(generateChildrenPages()).toSorted(
        comparator,
      )
      const sorted2 = structuredClone(generateChildrenPages()).toSorted(
        comparator,
      )
