...
describe(getTransformedPageContent, () => {
  ...
  it("adds ids only to level-2 prose headings without ids", () => {
    ...
    const proseContent =
      proseBlock?.type === "prose" ? proseBlock.content : undefined
    expect(proseContent).toBeDefined()
    const headingInfo = proseContent!.map((node) => {
      if (node.type !== "heading") return undefined
      return { level: node.attrs.level, id: node.attrs.id }
    })

    expect(headingInfo).toStrictEqual([
      {
        level: 2,
        id: expect.stringMatching(idPattern),
      },
      { level: 3, id: undefined },
      { level: 2, id: "existing-id" },
    ])
  })
