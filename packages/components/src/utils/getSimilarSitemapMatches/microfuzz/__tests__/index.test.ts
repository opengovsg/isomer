...
describe("createFuzzySearch", () => {
  const matches = (
    ...
    expectedIndices: HighlightRanges | null = null,
  ) => {
  ...
  it(`can match by: contains letters awww yisss fuzzzzz`, () => {
    ...
    matches("Nozbe.com web site", "website", null)
    matches("Książka 10 steps EN", "10en", s(w + ms + ms))
    matches("Won't fix", "wontfix", null)
  ...
  const noMatch = (text: string, query: string) => {
    const results = createFuzzySearch([text])(query)
  ...
  it(`can search by many keys`, () => {
    ...
    expect(
      createFuzzySearch([u1, u2, u3, u4], {
        getText: (item: { name: string; alias?: string | null }) => [
          item.name,
          item.alias,
        ],
      })("foo"),
