...
// NOTE: Shared by getCollectionItems (cards) — resolves an item's raw
// `dateTagged` against the parent's `tagCategories` into (a) the raw
// values needed for filter matching (see getFilteredItems) and (b) the
// pre-resolved display data for the card's status pill + date text (see
// CollectionCard). Mirrors getPillAndPlaintextTags' single-pass shape for
// the text-filter equivalent.
