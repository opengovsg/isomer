...
import { createPageSchema, getEditPageSchema, updatePageBlobSchema, updatePageSchema } from "~/schemas/page"
...
export const pageRouter = router({
  ...
  updatePage: protectedProcedure.input(updatePageSchema).query(async ({ input, ctx }) => {
    const parentId = ""
    const pageName = ""
  ...
  updatePageBlob: protectedProcedure.input(updatePageBlobSchema).query(async ({ input, ctx }) => {
  ...
  createPage: protectedProcedure.input(createPageSchema).query(async ({ input, ctx }) => {
    return { pageId: "" }
  })
