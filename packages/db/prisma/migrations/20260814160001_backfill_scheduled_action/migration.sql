...
--
-- Split into its own migration (separate transaction) from the preceding
-- AddColumn so the ACCESS EXCLUSIVE lock taken by ALTER TABLE ADD COLUMN is
-- released immediately, instead of being held for the duration of this
-- UPDATE against the hot "Resource" table.
